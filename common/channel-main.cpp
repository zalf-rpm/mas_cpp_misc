    ...
    for (auto i = 0; i < noOfChannels; i++) {
      ...
      KJ_IF_MAYBE (anyOut, startupInfoWriterClient) {
        ...
        KJ_IF_MAYBE (req, infoReq) {
          ...
          if (outputSturdyRefs && channelSRUrl.size() > 0)
        ...
        if (outputSturdyRefs && channelSR.size() > 0)
          std::cout << "channelSR=" << channelSR.cStr() << std::endl;
      }
      ...
      for (auto k = 0; k < noOfReadersPerChannel; k++) {
        ...
        KJ_IF_MAYBE (info, startupInfo) {
          ...
          if (outputSturdyRefs && readerSRUrl.size() > 0)
          ...
          if (outputSturdyRefs && readerSR.size() > 0)
            std::cout << "\treaderSR=" << readerSR.cStr() << std::endl;
        }
      }
      ...
      for (auto k = 0; k < noOfWritersPerChannel; k++) {
        ...
        KJ_IF_MAYBE (info, startupInfo) {
          ...
          if (outputSturdyRefs && writerSRUrl.size() > 0)
          ...
          if (outputSturdyRefs && writerSR.size() > 0)
            std::cout << "\twriterSR=" << writerSR.cStr() << std::endl;
        }
      }
...
private:
  ...
  struct ChannelData {
    ...
    ChannelData(AnyPointerChannel::Client client, Channel *channel) : client(kj::mv(client)), channel(channel) {}
  };
