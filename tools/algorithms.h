...
std::vector<std::string> splitString(const std::string& s,
                                     ...
                                     const std::pair<std::string, std::string>& tokenDelimiters =
                                       std::make_pair("", ""),
...
*/
template <class Collection>
std::pair<typename Collection::value_type, typename Collection::value_type> minMax(const Collection& xs);
...
//this is dangerous as it doesn't take of overflows
...
std::optional<T> round_to_digits(T value, int digits) {
  using Wide = long double;
  const Wide v = static_cast<Wide>(value);

  if (digits >= 0) return value;

  const Wide factor = std::pow(Wide(10), static_cast<Wide>(-digits));
  if (!std::isfinite(factor) || factor == 0) return std::nullopt;

  const Wide rounded = std::round(v / factor) * factor;

  if (!std::isfinite(rounded) ||
      rounded < static_cast<Wide>(std::numeric_limits<T>::lowest()) ||
      rounded > static_cast<Wide>(std::numeric_limits<T>::max())) {
    return std::nullopt;
  }
...
std::optional<T> round_to_digits(T value, int digits) {
  ...
  const Wide factor = std::pow(Wide(10), static_cast<Wide>(std::abs(digits)));
  if (!std::isfinite(factor) || factor == 0) return std::nullopt;

  Wide rounded = 0;

  if (digits >= 0) {
    if (std::fabs(v) > std::numeric_limits<Wide>::max() / factor) return std::nullopt;

    rounded = std::round(v * factor) / factor;
  } else {
    rounded = std::round(v / factor) * factor;
  }
...
inline double round(double value, int roundToDigits) {
  if (const auto v = round_to_digits(value, roundToDigits)) return *v;
  return value;
}

inline double round(double value) { return round(value, 0); }
...
Collection Tools::linearRegression(const Collection& xs, const Collection& ys) {
  ...
  Collection ys_r(n);
  for (int i = 0; i < n; i++) ys_r[i] = a_r + (b_r * xs.at(i));
...
Collection Tools::cumulativeSum(const Collection& ys, int n) {
  ...
  T acc = 0;
  Collection yis(n);
  for (int i = 0; i < n; i++) yis[i] = (acc += ys[i]);
...
Collection Tools::expGlidingAverage(const Collection& ys,
  ...
  if (ys.empty()) return Collection();

  Collection yas(ys.size());
  T ya = yas[0] = ys.first();
  for (int i = 1; i < ys.size(); i++) ya = yas[i] = (alpha * ys.at(i)) + ((1 - alpha) * ya);
...
Collection Tools::simpleGlidingAverage(const Collection& ys, int n) {
  ...
  if (ys.size() < n) return Collection();

  Collection yas(ys.size() - (2 * lr));
  T sum = 0;
  for (int i = 0; i < n; i++) sum += ys.at(i);
...
template <class Collection>
std::pair<typename Collection::value_type, typename Collection::value_type> Tools::minMax(const Collection& vs) {
  typedef typename Collection::value_type T;
...
double Tools::standardDeviation(const Collection& xs, bool calcAvg,
  ...
  for (auto x : xs) sum += (double(x) - xavg) * (double(x) - xavg);
...
Vector1 Tools::elemVecOp(const Vector1& left, const Vector2& right, OP op,
  ...
  if (!autoFit && int(left.size()) != int(right.size())) return Vector1();

  Vector1 res(msize);
  for (int i = 0; i < msize; i++) res[i] = op(left.at(i), right.at(i));
...
Vector1& Tools::inElemVecOp(Vector1& left, const Vector2& right, OP op,
  ...
  if (!autoFit && int(left.size()) != int(right.size())) return left;

  for (int i = 0; i < msize; i++) left[i] = op(left.at(i), right.at(i));
...
Vector Tools::scalVecOp(const Vector& left, T right, OP op) {
  ...
  for (int i = 0, size = left.size(); i < size; i++) res[i] = op(left.at(i), right);
  return res;
}

template <class Vector, typename T, class OP>
Vector& Tools::inScalVecOp(Vector& left, T right, OP op) {
  for (int i = 0, size = left.size(); i < size; i++) left[i] = op(left.at(i), right);
  return left;
}
...
int Tools::findThermalVegetationalSeasonStart(const DoubleVector& ts) {
  ...
  for (int i = 0, size = ts.size(); i < size; i++) {
    if (ts.at(i) > tresh) {
      double sum = 0;
      for (int k = i + 1; k < size && k < i + length; k++) sum += ts.at(k) - tresh;
      if (sum > 0) return i;
    }
  }
  return 365;
...
int Tools::findThermalVegetationalSeasonEnd(const DoubleVector& ts, int startDay) {
  ...
  for (int i = startDay, size = ts.size(); i < size; i++) {
    if (ts.at(i) < tresh) {
      double sum = 0;
      for (int k = i + 1; k < size; k++) sum += ts.at(k) - tresh;
      if (sum < 0) return i;
    }
  }
  return 365;
...
std::pair<int, int> Tools::forestalVegetationalSeasonStartEnd(Iterator begin,
  ...
  for (Iterator it = begin; it != end; it++, i++) {
    ...
    if (start == 0 && tavg > threshold) start = i + 1;
...
Collection Tools::classifyClimateZonesForPNV(const Collection& qs, PNVQ pnvq) {
  ...
  for (int i = 0, size = qs.size(); i < size; i++) {
    ...
    switch (pnvq) {
    case Qyr: if (q <= 10) res.push_back(1);
      else if (10 < q && q <= 20) res.push_back(2);
      else if (20 < q && q <= 30) res.push_back(3);
      else if (30 < q) res.push_back(4);
      break;
    case Qgs: if (q <= 20) res.push_back(1);
      else if (20 < q && q <= 40) res.push_back(2);
      else if (40 < q && q <= 53) res.push_back(3);
      else if (53 < q) res.push_back(4);
      break;
    }
  }
  return res;
